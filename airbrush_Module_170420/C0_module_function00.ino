...
byte Module_action(byte act_mode_number){
  ...
  static byte act_status = START_UP;
    ...
    switch(act_mode_number){
      ...
      case R2H:
        act_status = Ret2home(Target_direction, Target_speed);
      break;
    
      case MOVE_INC:
        act_status = Move_inc(Target_speed, Target_direction, Target_length);
      break;
    
      case MOVE_ABS:
       act_status = Move_abs(Target_speed, Target_position);
      break; 
    
      case MOVE_RCP:
        act_status = Move_rcp(Target_cycle_number, Target_speed, Target_startpos,Target_endpos);
      break; 
    
      default:
        Serial.write("default ");//debug
       act_status = TEST_ERR; //★
      break;      
    }
  return act_status;
